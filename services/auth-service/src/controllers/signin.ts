...
import { BadRequestError, IAuthDocument, isEmail } from '@kmnvz-mayvez/myapp-share';
...
export async function read(req: Request, res: Response): Promise<void> {
  ...
  const passwordsMatch: boolean = await (AuthModel.prototype as any).comparePassword(password, existingUser.password);
  if (!passwordsMatch) {
    throw new BadRequestError('Invalid credentials', 'SignIn read() method error');
  }
